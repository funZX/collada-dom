...
daeInt daeTinyXMLPlugin::writeToMemory(std::vector<char>& output, daeDocument *document)
{
	assert(false);
	return 0;
}
...
daeInt daeTinyXMLPlugin::write(const daeURI& name, daeDocument *document, daeBool replace)
{
    ...
    if (m_doc)
    {
		tinyxml2::XMLDeclaration* decl = m_doc->ToDeclaration();
...
void daeTinyXMLPlugin::writeValue( daeElement* element )
{
    if (daeMetaAttribute* attr = element->getMeta()->getValueAttribute()) {
		...
		if (!s.empty())
		{
			tinyxml2::XMLElement* tiElm = m_doc->NewElement(buffer.str().c_str());

			m_elements.push_front(tiElm);
			m_elements.front()->LinkEndChild(m_doc);
		}            
    }
}
